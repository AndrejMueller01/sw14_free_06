...
public class SafeFeatureTests extends
	...
	public void testProgressSafed() throws Exception {
		...
		ArrayList<Course> currCourses = XMLParser.getInstance(null)
				.getCurrentCourses();
		for (int i = 0; i < currCourses.size(); i++) {
			if (text.equals(currCourses.get(i).getCourseName())) {
				assertEquals(GlobalProperties.STATUS_DONE, currCourses.get(i)
						.getStatus());
			}
		}
	...
	public void testProgressSafedMoreCourses() throws Exception {
		...
		for (int i = 0; i < currCourses.size(); i++) {
			if (text2.equals(currCourses.get(i).getCourseName())) {
				assertEquals(GlobalProperties.STATUS_IN_PROGRESS, currCourses
						.get(i).getStatus());
			}
		}
	...
	public void testAllCoursesDone() throws Exception {
		...
		EditText ectsET = (EditText) solo
				.getView(R.id.create_course_ects_edit_text);
		...
		assertEquals(180.0f, XMLParser.getInstance(null).getCurrentEcts());
	}

	public void testDeleteFirstCourse() throws Exception {
	...
	public void testDeleteOneCourse() throws Exception {
		solo.waitForActivity(ChooseStartConfigurationActivity.class);
		...
		solo.clickOnCheckBox(middle);
		int end = sem1lv.getAdapter().getCount() - 1;
		solo.clickOnCheckBox(end);
		solo.clickOnMenuItem("Done");
		solo.goBack();
		solo.clickOnButton("Ja");
		...
		for (int i = 0; i < courseNamesAfter.length; i++) {
			...
			if (i == 0 || i == middle-1 || i == end-1)
				offsetCounter++;

			assertEquals(courseNames[i + offsetCounter], courseNamesAfter[i]);
